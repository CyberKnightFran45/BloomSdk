...
ReadOnlySpan<char> rawLine = vLine.AsSpan();
...
using var vNext = nextLine.Value;
ReadOnlySpan<char> nextTrimmed = vNext.AsSpan().Trim();
...
currentBlock.AsSpan()[blockLength++] = '\n';
}

EnsureCapacity(ref currentBlock, blockLength + rawLine.Length);

rawLine.CopyTo(currentBlock.AsSpan( (ulong)blockLength) );
...
currentValue.AsSpan()[strLen++] = '\n';
}

EnsureCapacity(ref currentValue, strLen + rawLine.Length);

rawLine.CopyTo(currentValue.AsSpan( (ulong)strLen) );
