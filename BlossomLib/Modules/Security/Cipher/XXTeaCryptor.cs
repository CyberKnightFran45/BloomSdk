...
<remarks> Original Algorithm by David Wheeler, Roger Needham and Ma Xiaoyun </remarks> */
...
var uSpan = result.AsSpan();

if(includeLength)
uSpan[n] = (uint)length;
       
for(int i = 0; i < length; i++)
uSpan[i >> 2] |= (uint)data[i] << ( (i & 3) << 3);
...
var bSpan = result.AsSpan();

for(int i = 0; i < n; i++) 
bSpan[i] = (byte)(data[i >> 2] >> ( (i & 3) << 3) );
...
using NativeMemoryOwner<uint> vOwner = ToUInts(input, true);

var v = vOwner.AsSpan();
int n = v.Length - 1;

using NativeMemoryOwner<uint> kOwner = ToUInts(key, false);
var k = kOwner.AsSpan();
...
using NativeMemoryOwner<uint> vOwner = ToUInts(input, false);
