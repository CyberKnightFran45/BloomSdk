...
try
{
using var pOwner = InputHelper.GetNativeString(bOwner.AsSpan() );
bOwner.Dispose();

var plainText = pOwner.AsSpan();
...
target[.. dataLen].CopyTo(str.AsSpan() );

var buffer = str.AsSpan();
...
if(padding > 0)
buffer.Slice(dataLen, padding).Fill('=');
...
target[.. dataLen].CopyTo(bytes.AsSpan() );
...
if(padding > 0)
buffer.Slice(dataLen, padding).Fill(0x3D); // Add '='
