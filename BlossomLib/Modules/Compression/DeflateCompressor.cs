...
{
/// <summary> Initializes Compression for Files by using the Deflate algorithm. </summary>
...
{
using DeflateStream dflStream = new(output, level, true);

FileManager.Process(input, dflStream, maxBytes, progressCallback);
}
...
{
TraceLogger.Init();
TraceLogger.WriteLine("Deflate Compression Started");

long originalSize = 0;

try
{
PathHelper.AddExtension(ref outputPath, ".dfl");
...
TraceLogger.WriteLine("Deflate Compression Finished");
...
Action<long, long> progressCallback = null)
{
using DeflateStream inflater = new(input, CompressionMode.Decompress, true);

FileManager.Process(inflater, output, maxBytes, progressCallback);
}
...
{
TraceLogger.Init();
TraceLogger.WriteLine("Deflate Decompression Started");
...
TraceLogger.WriteLine("Deflate Decompression Finished");
