...
int highBits = mortonIndex >> (2 * logMin) << (2 * logMin);
 
int combinedIndex = isWide ? CombineIndex(highBits, mortonY, mortonX, minDimension, logMin) :
                             CombineIndex(highBits, mortonX, mortonY, minDimension, logMin);

int x = isWide ? combinedIndex / minDimension : combinedIndex % minDimension;
int y = isWide ? combinedIndex % minDimension : combinedIndex / minDimension;
