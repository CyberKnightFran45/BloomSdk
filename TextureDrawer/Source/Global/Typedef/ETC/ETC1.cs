...
for(int i = 0; i < 8; i++)
{
int deltaL = TextureHelper.ColorClamp(MOD_TABLE[i, 0] * 2);
int deltaM = TextureHelper.ColorClamp(MOD_TABLE[i, 0] + MOD_TABLE[i, 1]);
int deltaH = TextureHelper.ColorClamp(MOD_TABLE[i, 1] * 2);
...
{
data = (data & ~(1uL << pos) ) | ( (mode ? 1uL : 0uL) << pos);
}
...
{
data = (data & ~(7uL << pos) ) | ((ulong)table & 7uL) << pos;
}
...
private static void GenPixDiff(ref ulong data, ReadOnlySpan<TextureColor> pixels, in TextureColor baseColor,
                               int modifier, int startX, int endX, int startY, int endY)
{
...
private static ulong PackBits(int flags, int factor) => (ulong)(flags / 0x11) << factor;

// Set Table Index

private static ulong SetTableIndex(int flags, int factor) => (ulong)(flags & 0x7) << factor;
...
public static int Encode(Stream writer, SKBitmap image, Endianness endian = default)
{
return ETC.Encode(writer, image, endian, EncodeColor);
}
...
private static int ExtractColor(ulong flags, int factor) => (int)( (flags >> factor) & 0x1F);

// Expand Color bits

private static int ExpandBits(int flags) => (flags << 3) | ( (flags & 0x1C) >> 2);

// Update Color from flags

private static int UpdateColor(ulong flags, int factor) => (int)( (flags >> factor) & 0x7) << 29 >> 29;

// Unpack Color bits

private static int UnpackBits(ulong flags, int factor) => (int)( (flags >> factor) & 0xF) * 0x11;

// Get Table Index

private static int GetTableIndex(ulong flags, int factor) => (int)( (flags >> factor) & 0x7);
...
public static SKBitmap Decode(Stream reader, int width, int height, Endianness endian = default)
{
return ETC.Decode(reader, width, height, endian, DecodeColor);
}
