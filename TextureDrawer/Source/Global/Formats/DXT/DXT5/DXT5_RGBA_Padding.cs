...
public static SKBitmap Read(Stream reader, int width, int height, int blockSize, Endianness endian = default)
{
...
public static int Write(Stream writer, SKBitmap image, int blockSize, Endianness endian = default)
{
TraceLogger.WriteLine("• DXT5-RGBA-Padding Texture Encode:");
TraceLogger.WriteLine();

using ChunkedMemoryStream encodedStream = new();
_ = DXT.Encode(encodedStream, image, endian, false, null, DXT4_RGBA.EncodeAlpha);
